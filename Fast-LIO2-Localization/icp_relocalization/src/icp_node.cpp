...
public:
        ...
        : Node("icp_node")
    {
        ...
        this->declare_parameter("fitness_score_thre", 0.5);
        ...
        else
        {
            ...
            "/velodyne_points", 10, std::bind(&ICPNode::cloud_callback, this, std::placeholders::_1));
        }
#else
        cloud_sub_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
            "/velodyne_points", 10, std::bind(&ICPNode::cloud_callback, this, std::placeholders::_1));
...
private:
    void cloud_callback(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
    {
        ...
        // [추가됨] 1.5. Velodyne -> IMU(Body) 좌표계 변환 (Extrinsic 적용)
        // 매핑 때 사용한 값: extrinsic_T: [0.0, 0.0, 0.20131], R: Identity
        Eigen::Affine3f sensor_to_body = Eigen::Affine3f::Identity();
        sensor_to_body.translation() << 0.0, 0.0, 0.20131; // X, Y, Z 오프셋
        // 만약 회전도 있었다면: sensor_to_body.rotate(...); 
        
        // input_cloud(센서 기준)를 input_cloud(IMU 기준)으로 변환
        pcl::transformPointCloud(*input_cloud, *input_cloud, sensor_to_body);
